...
import { JCheckSession, JConnectResponse, JInitializeResponse, JOpnCmpnyResponse, JValidateResponse, UploadAttachmentResposne } from "../Entity/JInitializeResponse"
...
export default class ERESApi extends BaseApi {
    ...
    public static async JValidate(model: { objUsr: { sName?: string, sCode?: string } }): Promise<JValidateResponse> {
        return await this.Post("ERES", `Sys/Sys.aspx/JValidate`, model)
    }
    public static async UploadAttachment(model: FormData): Promise<UploadAttachmentResposne> {
    ...
    public static async DownloadAttachment(AttachmentId: number): Promise<any> {
        return await this.Post("ERES", 'SYS/Sys.aspx/JGetAttch', { lId: AttachmentId })
    }
}
