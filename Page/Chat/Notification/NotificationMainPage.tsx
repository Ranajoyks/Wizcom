...
const NotificationMainPage = () => {
  ...
  const UpdateAllOnlineUser = async (allUsers: NotificationUser[]) => {
    ...
    dispatch(
      ChatUserOptions.actions.UpdateFilterNotificationUserList(allUsers),
    );
  ...
  const FetchAllUserNotificationMessages = async () => {
    ...
    var cuResponse = await SignalRApi.GetAllUserNotification(
      tempSenderChatId!,
      branch?.lId!,
    );
    ...
    if (cuResponse.data) {
      ...
      dispatch(
        ChatUserOptions.actions.UpdateAllNotificationUserList(cuResponse.data),
      );
      cuResponse.data.forEach(user => {
        dispatch(
          ChatUserOptions.actions.LoadUserOneToOneNotificationChatList({
          ...
          <FlatList
            ...
            renderItem={d => {
              return (
                <ChatUserScreen
                  data={d.item}
                  OnUserListRefresRequest={FetchAllUserNotificationMessages}
                />
              );
            }}
...
const ChatUserScreen = (props: {
  ...
  var unreadMessageList = user.AllNotificatonOneToOneList?.filter(
    i => !i.bStatus && i.lReceiverId != user.lId,
  );
  var lastMessage = user.AllNotificatonOneToOneList?.length
    ? user.AllNotificatonOneToOneList[0]
    : undefined;
