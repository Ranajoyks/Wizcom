...
const OneToOneChatOptions = createSlice({
  ...
  reducers: {
    UpdateAllUserListAndMessage: (state, action: PayloadAction<ChatUser[]>) => {
        console.log("action.payload/: ----",action.payload.slice(0,2));
        

      action.payload.forEach(payloadUser => {
        var oldUserIndex = state.AllUserList.findIndex(
        ...
        var newNoProxySortedMessageList: Chat[] =
          newUser.AllChatOneToOneList.filter(i => !i.IsKsProxy);
    ...
    //   console.log('AllUserList: ', state.AllUserList.slice(0, 3));
    },
    ...
    UpdateOneToOneChat: (state, action: PayloadAction<Chat>) => {
      var userIndex = state.AllUserList.findIndex(
        i =>
          i.lId == action.payload.lReceiverId ||
          i.lId == action.payload.lSenderId,
      );
      var user = state.AllUserList[userIndex];
      var chatIndex = user.AllChatOneToOneList?.findIndex(
        i => i.lSrId == action.payload.lSrId,
      )!;

      state.AllUserList[userIndex].AllChatOneToOneList![chatIndex] =
        action.payload;
    },
  },
...
export const GetFilteredUserList = createSelector(
  ...
  (AllUserList, mheader) => {
    ...
    var tempFilteredUserList = tempFilteredUserList.filter(i =>
      i.userName
        .toLowerCase()
